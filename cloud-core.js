(function(global){
  ...
  function injectDock(){
    ...
    const style=document.createElement('style');style.textContent=`
      #solarCloudDock{position:relative;z-index:9998;display:flex;align-items:center;gap:7px;width:max-content;max-width:100%;margin:0 auto;padding:7px 9px;background:#102432ee;color:#e8f3f5;border:1px solid #426171;border-radius:12px;box-shadow:0 8px 22px #0003;font:500 11px 'Bai Jamjuree',sans-serif;backdrop-filter:blur(10px);overflow-x:auto;flex-shrink:1}
      @media(max-width:980px){#solarCloudDock{order:10;width:100%;margin-top:8px;justify-content:center}header{flex-wrap:wrap}}
    ...
    const pageHeader=document.querySelector('header'),headerControls=pageHeader?.querySelector('.header-controls');if(pageHeader)pageHeader.insertBefore(dock,headerControls||null);else document.body.prepend(dock);setLanguage(interfaceLanguage);
