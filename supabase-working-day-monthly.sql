-- FusionSolar Working Day monthly loader
-- Safe additive update. No existing records are changed.
...
begin
  ...
  from (select distinct to_char(record_date,'YYYY-MM') m
        from public.central_daily_records where workspace_id=p_workspace) q;

  v_month=coalesce(nullif(p_month,''),v_months->>0);
  if v_month is null then
    return jsonb_build_object('records','[]'::jsonb,'detectedMonths',v_months,'availableMonths',v_months,'loadedMonth',null);
  end if;
  ...
  where r.workspace_id=p_workspace and r.record_date>=v_start and r.record_date<v_end;
...
select 'FusionSolar Working Day monthly loader installed successfully' as result;
